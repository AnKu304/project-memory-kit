...
class InstallerNewRepoTest(unittest.TestCase):
    def test_installs_memory_boilerplate_and_runtime(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue((root / ".project-memory/evals").exists())
            ...
            self.assertIn("evals_dir: .project-memory/evals", config)
            self.assertIn("human_dir: .project-memory/human", config)
            self.assertIn("human:", config)
            self.assertIn("enabled: false", config)
            self.assertIn("integrations:", config)
            self.assertIn("linear:", config)
            self.assertIn("bridge_dir: .project-memory/linear", config)
            self.assertIn("parsers:", config)
            self.assertIn("backend: auto", config)
