...
Из **точного корня существующей памяти** (контейнера, если код во вложенном Git):
...
Команды добавляют `project_memory` в `.mcp.json` и `.kimi-code/mcp.json`.
Оба клиента используют тот же `pmem` и ту же БД, без новой индексации или
запуска моделей. Существующие другие серверы сохраняются; конфликтующий
`project_memory`, некорректный JSON и symlink отклоняются без перезаписи.
Без `--write` выводится JSON для этих клиентов, TOML для Codex/generic.
MCP-конфиги и `.kimi-code/` исключены из индекса как потенциально приватные.

Запускайте CLI именно из этого корня: Kimi ищет проектный конфиг относительно
рабочей папки. Он читает `AGENTS.md` и `.agents/skills/`; не копируйте всю память
в промпт. Claude использует `CLAUDE.md`: добавьте профиль `claude` штатным upgrade
после проверки локальных правок, либо в существующем `CLAUDE.md` явно сошлитесь
на общий `AGENTS.md` и навык PMEM, не заменяя пользовательские инструкции.
Все служебные файлы остаются в контейнере, не во вложенном продуктовом Git.

Начните новый сеанс и подтвердите доверие штатно, затем `/mcp` в каждом клиенте.
Первая проверка: `pmem_doctor` → точный root/БД; `pmem_context` → небольшая
реальная задача; show известной записи. Сохранение подтверждается только
`saved`, `completed: true`, ID и последующим show/search. Локальная память
не отменяет передачу выбранного контекста провайдеру используемой модели.

Авторизация CLI, подписка/квота и разрешения проверяются отдельно. Эти команды
не входят в аккаунты, не меняют Keychain, не включают API-биллинг и не обходят
trust/approval. При запрете Keychain не запускайте непроверенный Claude OAuth;
наличие конфига не доказывает готовность живого клиента. Для делегирования
действуют общие правила бюджета, выбора модели и независимой проверки.

Форматы: [Claude MCP](https://code.claude.com/docs/en/mcp),
[Kimi MCP](https://www.kimi.com/code/docs/en/kimi-code-cli/customization/mcp.html).
Kimi здесь — `@moonshot-ai/kimi-code` (`.kimi-code`), не отдельный Python `kimi-cli`.
...
- `0.23.2`: проектные MCP-конфиги Claude/Kimi, безопасное сохранение настроек и исключение приватных конфигов из индекса.
